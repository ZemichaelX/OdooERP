...
# is stopped during the restore. Confirm the target name carefully.
set -euo pipefail
...
if [ -n "${FILESTORE_TGZ}" ]; then
  ...
  $COMPOSE exec -T odoo sh -c "rm -rf /var/lib/odoo/filestore/${DB_NAME}; mkdir -p /var/lib/odoo/filestore"
  $COMPOSE exec -T odoo tar xzf - -C /var/lib/odoo/filestore < "${FILESTORE_TGZ}"
fi
